...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateInputHandlers() {
    ...
    if (currentMode === "explain") {
      ...
      startExplain.addEventListener("click", () =>
        startGuidance(goalInput.value.trim(), "explain")
      );
    } else if (currentMode === "do") {
      taskInput.addEventListener("input", handleDoInput);
      startDo.addEventListener("click", () =>
        startGuidance(taskInput.value.trim(), "do")
      );
    } else if (currentMode === "error") {
      errorInput.addEventListener("input", handleErrorInput);
      startErrorHelp.addEventListener("click", () =>
        startGuidance(errorInput.value.trim(), "error")
      );
    }
  }
  ...
  function startGuidance(input, mode) {
    ...
    statusSection.querySelector(
      ".status-text"
    ).textContent = `Analyzing your ${mode} request...`;
    ...
    chrome.runtime.sendMessage(
      ...
      function (response) {
        if (response && response.success) {
          statusSection.querySelector(
            ".status-text"
          ).textContent = `${mode} mode started! Check the AWS Console.`;
          ...
          statusSection.querySelector(
            ".status-text"
          ).textContent = `Error starting ${mode} mode. Please try again.`;
        }
      }
    );
  }
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    if (!isAWSConsole) {
      statusSection.style.display = "block";
      statusSection.querySelector(".status-text").textContent =
        "Please navigate to AWS Console first.";
