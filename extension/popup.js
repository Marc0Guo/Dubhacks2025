...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // AWS Configuration elements
  const awsConfigButton = document.getElementById('aws-config-btn');
  const awsConfigSection = document.getElementById('aws-config-section');
  const saveAwsConfigButton = document.getElementById('save-aws-config');
  ...
  // Handle AWS config button
  awsConfigButton.addEventListener('click', function() {
    awsConfigSection.style.display = awsConfigSection.style.display === 'none' ? 'block' : 'none';
  });

  // Handle AWS config save button
  saveAwsConfigButton.addEventListener('click', function() {
    ...
    chrome.runtime.sendMessage({
      ...
      if (response && response.success) {
        ...
        awsConfigSection.style.display = 'none';
      } else {
        alert('Error saving AWS credentials');
      }
    });
  });
  ...
  // Load existing AWS credentials on popup open
  chrome.runtime.sendMessage({ action: 'getAWSCredentials' }, function(response) {
    if (response && response.success && response.credentials) {
      document.getElementById('aws-access-key').value = response.credentials.accessKeyId || '';
      document.getElementById('aws-secret-key').value = response.credentials.secretAccessKey || '';
      document.getElementById('aws-region').value = response.credentials.region || 'us-east-1';
    }
  });
});
