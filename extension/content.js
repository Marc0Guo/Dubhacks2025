...
function activateExplainMode() {
  console.log('Activating explain mode');
  ...
  console.log('Explain mode activated - click any element to get an explanation');
}

// Handle element clicks in explain mode
function handleElementClick(event) {
  ...
  // Skip certain elements that shouldn't be explained
  ...
  // Send to background script for explanation
  chrome.runtime.sendMessage({
    action: 'explainElement',
    elementData: elementData
  }, (response) => {
    if (response && response.success) {
      console.log('Element explanation received');
    } else {
      console.error('Failed to get element explanation:', response);
...
function shouldSkipElement(element) {
  // Skip elements that are part of our extension
  if (element.closest('#aws-navigator-overlay') ||
      element.closest('#aws-navigator-panel') ||
      element.closest('#aws-navigator-general-panel') ||
      element.closest('#aws-navigator-cue') ||
      element.closest('#aws-navigator-explanation-panel')) {
    return true;
  }
...
function getElementContext(element) {
  ...
  while (parent && depth < 3) {
    if (parent.textContent?.trim()) {
      context.push(`${parent.tagName}: ${parent.textContent.trim().substring(0, 100)}`);
    }
...
function showExplainModeIndicator() {
  ...
  const existingIndicator = document.getElementById('aws-navigator-explain-indicator');
  if (existingIndicator) {
    existingIndicator.remove();
  }

  const indicator = document.createElement('div');
  indicator.id = 'aws-navigator-explain-indicator';
  ...
  indicator.innerHTML = `
    <div style="display: flex; align-items: center; gap: 8px;">
      ...
      <button id="aws-navigator-stop-explain" style="background: rgba(255,255,255,0.2); border: none; color: white; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px;">Stop</button>
    </div>
  `;
  ...
  const stopBtn = document.getElementById('aws-navigator-stop-explain');
  stopBtn.addEventListener('click', (e) => {
    e.stopPropagation();
...
function deactivateExplainMode() {
  console.log('Deactivating explain mode');
  ...
  // Remove indicator
  const indicator = document.getElementById('aws-navigator-explain-indicator');
  if (indicator) {
    indicator.remove();
  }

  // Remove any existing explanation panels
  const existingPanel = document.getElementById('aws-navigator-explanation-panel');
  if (existingPanel) {
    existingPanel.remove();
  }
...
function showLoadingIndicator(element) {
  ...
  loadingIndicator.id = 'aws-navigator-loading-indicator';
  ...
  // Add pulse animation
  if (!document.getElementById('aws-navigator-loading-styles')) {
    const style = document.createElement('style');
    style.id = 'aws-navigator-loading-styles';
    style.textContent = `
      @keyframes pulse {
...
function hideLoadingIndicator() {
  const loadingIndicator = document.getElementById('aws-navigator-loading-indicator');
  if (loadingIndicator) {
...
function showElementExplanation(explanation, elementData, isFallback = false) {
  ...
  // Remove existing explanation panel
  const existingPanel = document.getElementById('aws-navigator-explanation-panel');
  if (existingPanel) {
    existingPanel.remove();
  }
  ...
  const panel = document.createElement('div');
  panel.id = 'aws-navigator-explanation-panel';
  panel.style.cssText = `
    position: fixed;
    top: 50%;
    left: 50%;
  ...
  // Add close handlers
  const closeBtn = document.getElementById('aws-navigator-close-explanation');
  const closeBtn2 = document.getElementById('aws-navigator-close-explanation-btn');
  ...
  closeBtn2.addEventListener('click', closeExplanation);
