#!/bin/bash
# Setup script for Python Bedrock Agent

echo "🚀 Setting up Python Bedrock Agent..."

# Set environment variables
export AWS_DEFAULT_REGION=us-east-1
export AWS_BEARER_TOKEN_BEDROCK="ABSKQmVkcm9ja0FQSUtleS1zbXNpLWF0LTk3MTQyMjY3MTEwMDo2VkxETUdTL1VRZU1FZHlXVkppbEJvNUtKcWdyWGpyZGQ4MG4wN3hTVlVaRGhZcUxUU2tFK0djREJMTT0="

# Install Python dependencies
echo "📦 Installing Python dependencies..."
pip3 install boto3 flask flask-cors

# Create a modified version that uses the bearer token
echo "🔧 Creating bearer token version..."

cat > src/app_with_bearer.py << 'EOF'
...
def call_bedrock(user_msg: str) -> str:
    if not BEDROCK_API_KEY:
        return "Error: AWS_BEARER_TOKEN_BEDROCK not set"
    
    # Use Titan Express model with bearer token
    ...
    try:
        response = requests.post(
            f"{BEDROCK_ENDPOINT}/model/{MODEL_ID}/invoke",
            headers={
        ...
        if response.status_code == 200:
            ...
            return f"Bedrock API error: {response.status_code} - {response.text}"
    except Exception as e:
        return f"Bedrock request failed: {str(e)}"
...
echo "✅ Python Bedrock Agent setup complete!"
echo ""
echo "🚀 To start the agent:"
echo "   cd /Users/anthonywen04/Desktop/Dubhacks2025/Bedrock_Agent"
echo "   ./start-agent.sh"
echo ""
echo "🔧 Or manually:"
echo "   export AWS_BEARER_TOKEN_BEDROCK=your-key"
echo "   export AWS_DEFAULT_REGION=us-east-1"
echo "   python3 start-agent.py"
