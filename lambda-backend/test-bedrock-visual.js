// Visual test script to show Bedrock success/failure clearly
import {
...
async function testBedrockVisual() {
  console.log("🚀 AWS Tutor - Bedrock Integration Test");
  ...
  try {
    ...
    if (planResult.success) {
      ...
      console.log(`   Steps: ${planResult.data.steps.length} steps`);
      console.log(`   Time: ${planResult.data.estimated_time}`);
      console.log(`   Difficulty: ${planResult.data.difficulty}`);
      console.log("\n📝 First 3 steps:");
...
// Run the test
testBedrockVisual().catch(console.error);
